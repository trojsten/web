...
from trojsten.rules.models import FKSLevel
from trojsten.submit.models import Submit

FKS_ALL = "FKS_ALL"
FKS_L1, FKS_L2, FKS_L3, FKS_L4 = "FKS_L1", "FKS_L2", "FKS_L3", "FKS_L4"


class FKSResultsGenerator(ResultsGenerator):
    ...
    def get_submit_queryset(self, res_request):
        ...
        submits = (
            super(FKSResultsGenerator, self)
    ...
    def source_or_zip_submit_points(self, previous_points, submit):
        ...
        Source and ZIP points in FKS can be computed from two submits, which demands this processing:
    ...
    def add_submit_to_row(self, res_request, submit, row):
        if (
            submit.submit_type == submit_constants.SUBMIT_TYPE_SOURCE
            or submit.submit_type == submit_constants.SUBMIT_TYPE_TESTABLE_ZIP
        ):
            cell = row.cells_by_key[submit.task.number]
            cell.auto_points = self.source_or_zip_submit_points(cell.auto_points, submit)
        else:
            super(FKSResultsGenerator, self).add_submit_to_row(res_request, submit, row)

    def deactivate_row_cells(self, res_request, row, cols):
        user_level = res_request.user_levels[row.user.pk]

        # Deactivate users who are in the results table of a higher level (L) but don't have
        # any submits for tasks L+3 and L+4. This makes level result tables cleaner but it doesn't
        # exclude any winners (because to win you have to have at least 150 points
        # from 200 possible and for that you have to have submits for these tasks).
        if self.tag.key != FKS_ALL and user_level < self.get_results_level():
            ...
            if not res_request.single_round:
                ...
                active_in_previous_round = (
                    False if row_from_previous_round is None else row_from_previous_round.active
                )

            task_cells = [row.cells_by_key[self.get_results_level() + i] for i in (3, 4)]
            current_round_points = sum(
                self.get_cell_total(res_request, cell) for cell in task_cells
            )
    ...
    def add_special_row_cells(self, request, row, cols):
        ...
        super(FKSResultsGenerator, self).add_special_row_cells(request, row, cols)
    ...
    def create_results_cols(self, res_request):
        ...
        for col in super(FKSResultsGenerator, self).create_results_cols(res_request):
            yield col
...
class FKSRules(CompetitionRules):
    RESULTS_TAGS = OrderedDict(
        [
            ...
            (FKS_L1, ResultsTag(key=FKS_L1, name="L1")),
            (FKS_L2, ResultsTag(key=FKS_L2, name="L2")),
            (FKS_L3, ResultsTag(key=FKS_L3, name="L3")),
            (FKS_L4, ResultsTag(key=FKS_L4, name="L4")),
        ]
    )
