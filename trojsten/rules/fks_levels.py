# -*- coding: utf-8 -*-
# Calculates history of FKS levels.
...
def prepare_events(latest_event_start_date):
    ...
    last_rounds = (
        Round.objects.filter(
            semester__competition__name="FKS", start_time__lte=latest_event_start_date
        )
    ...
    fks_site_id = Competition.objects.get(name="FKS").sites.first().id
    camp_objects = (
        Event.objects.filter(type__is_camp=True, start_time__lte=latest_event_start_date)
        .filter(type__sites__in=[fks_site_id])
        .order_by("start_time")
        .select_related("semester")
    )
...
def level_updates_from_semester_results(semester, level_up_score_thresholds=None):
    ...
    result_tables = [
        (get_results("FKS_L{}".format(level), round, single_round=False), level)
...
def level_updates_from_camp_attendance(
    ...
    user_levels = FKSLevel.objects.for_users_in_semester_as_dict(
    ...
    # TODO: Get frozen results table if available.
    results_table = get_results("FKS_ALL", last_round, single_round=False)
