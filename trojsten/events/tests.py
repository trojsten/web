...
class EventTest(TestCase):
    ...
    def test_existing_event(self):
        event = Event.objects.create(name="Sustredkovy event", type=self.event_type, place=self.place,
...
class EventParticipantsTest(TestCase):
    ...
    def test_user_list_exists(self):
        url = reverse('participants_list', kwargs={'event_id': self.event.id})
...
class EventRegistrationTest(TestCase):
    ...
    def setUp(self):
        ...
        article3 = Article.objects.create()
        urlpath3 = URLPath.objects.create(site=site, article=article3, slug=self.event.id,
                                          parent=urlpath2)
        ArticleRevision.objects.create(article=article3, title="Nazov1")

        article4 = Article.objects.create()
        urlpath4 = URLPath.objects.create(site=site, article=article4, slug="prihlaska",
                                          parent=urlpath3)
        ArticleRevision.objects.create(article=article4, title="Nazov1")
