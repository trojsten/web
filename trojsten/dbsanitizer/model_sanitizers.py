...
class RandomStringGenerator(object):
    def __init__(
        self, alphabet, length=None, min_length=None, max_length=None, mapping_function=None
    ):
...
class TaskSanitizer(BaseModelSanitizer):
    model = Task
    field_sanitizers = {"name": GeneratorFieldSanitizer(TitleGenerator())}
...
class UserPropertySanitizer(BaseModelSanitizer):
    model = UserProperty
    field_sanitizers = {"value": SmartStringSanitizer()}
