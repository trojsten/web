...
class UserResult(object):
    ...
    def add_task_points(self, task, submit_type, points, submit_status):
        if (
            ...
            or submit_constants.SUBMIT_TYPE_TEXT
        ):
            if (
...
def get_frozen_results(round, category=None, single_round=False):
    ...
    frozen_user_results = (
        frozen_results.frozenuserresult_set.all()
        .prefetch_related("task_points__task")
        .select_related("original_user", "school")
        .order_by("rank")
    )
...
def make_result_table(
    user, round, category=None, single_round=False, show_staff=False, force_generate=False
):
    ResultsTable = namedtuple("ResultsTable", ["tasks", "results_data", "has_previous_results"])

    if not (
        user.is_authenticated() and user.is_in_group(round.semester.competition.organizers_group)
    ):
        show_staff = False

    current_tasks = Task.objects.for_rounds_and_category([round], category).prefetch_related(
        "category"
    )
    ...
    if force_generate or not round.frozen_results_exists(single_round):
        ...
        if not single_round:
            previous_rounds = (
                Round.objects.visible(user, all_sites=True)
                .filter(semester=round.semester, number__lt=round.number)
                .order_by("number")
            )

            if previous_rounds:
                previous_tasks = Task.objects.for_rounds_and_category(previous_rounds, category)
        ...
        final_results = format_results_data(results, compute_rank=False)

    return ResultsTable(
        tasks=current_tasks, results_data=final_results, has_previous_results=has_previous_results
    )
...
def get_scoreboards_for_rounds(rounds, request):
    ...
    return [
        ResultTableObject(
            ...
            request.user.is_anonymous
            or request.user.is_valid_for_competition(round.semester.competition),
        )
