...
def get_results_data(tasks, submits):
    ...
    for submit in submits:
        ...
        if submit.submit_type == Submit.DESCRIPTION:
            ...
            res[submit.user][submit.task]['source'] += submit.normalized_points
        res[submit.user][submit.task]['sum'] += submit.normalized_points
        res[submit.user][submit.task]['submitted'] = True
        res[submit.user]['sum'] += submit.normalized_points
    return res
