...
class RecentResultsTest(TestCase):
    ...
    def test_dont_show_old_result(self):
        ...
        Round.objects.create(
            number=1, semester=semester1, solutions_visible=True, visible=True, end_time=bad_time
        )
        ...
        Round.objects.create(
            number=2, semester=semester2, solutions_visible=True, visible=True, end_time=good_time
        )
        ...
        competition_names = list(
            map(
                lambda x: x.scoreboard.round.semester.competition.name,
...
class ResultsTest(TestCase):
    def setUp(self):
        competition = Competition.objects.create(name="TestCompetition")
        competition.sites.add(Site.objects.get(pk=settings.SITE_ID))
        self.semester0 = Semester.objects.create(
            number=1, name="Test semester 2", year=0, competition=competition
        )
        ...
        self.semester2 = Semester.objects.create(
            number=2, name="Test semester 2", year=1, competition=competition
        )
        ...
        self.round0 = Round.objects.create(
            number=2,
            semester=self.semester0,
            solutions_visible=True,
            start_time=start0,
            end_time=end0,
            visible=True,
...
class SerializationTest(TestCase):
    ...
    def test_serialize_col_with_task(self):
        ...
        d = {"name": "1", "key": "T1", "task": {"id": task.id, "name": "Test task 1"}}
    ...
    def test_serialize_row_without_previous(self):
        ...
        school_dict = {"id": school.id, "name": "GJH", "verbose_name": "Gymnazium Jura Hronca"}
        user_dict = {
            "id": user.id,
            "username": "TestUser",
            "name": "Jozko Mrkvicka",
        ...
        d = {"user": user_dict, "cell_list": cell_list, "rank": 1, "active": True}
    ...
    def test_serialize_row_with_previous(self):
        ...
        prev_d = {"user": user_dict, "cell_list": prev_cell_list, "rank": 2, "active": True}
        d = {
            "user": user_dict,
            "cell_list": cell_list,
            "rank": 1,
    ...
    def test_serialize_results(self):
        ...
        cell_list_raw = [cell1, cell2]
        cell_list = [cell.serialize() for cell in cell_list_raw]
        row1_d = {"user": user_dict, "cell_list": cell_list, "rank": 1, "active": True}
        ...
        cell_list_raw = [cell1, cell2]
        cell_list = [cell.serialize() for cell in cell_list_raw]
        row2_d = {"user": user_dict, "cell_list": cell_list, "rank": 2, "active": True}
        row2 = ResultsRow(user, active=True)
        row2.cell_list = cell_list
        row2.rank = 2

        d = {"cols": [col_d], "rows": [row1_d, row2_d]}
