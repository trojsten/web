...
class CreateDiplomaTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.form_data = urlencode(
            {"template": 1, "participants_data": "fieldname\nfieldvalue\n", "join_pdf": "on"}
        )
    ...
    def test_malformed_input(self):
        ...
        malformed = urlencode(
            {"template": 1, "participants_data": '[{"name": "val]', "join_pdf": "on"}
        )
        r = self.client.post(
            self.url, data=malformed, content_type="application/x-www-form-urlencoded"
        )
