...
def diploma_sources(request, diploma_id):
    ...
    for source in diploma.sources.all():
        ...
        sources.append({"html": src.render(), "name": src.name, "verbose_name": source.name})
...
def view_diplomas(request):
    ...
    context = {"form": form, "template_fields": json.dumps(editable_fields, ensure_ascii=False)}

    return render(request, "trojsten/diplomas/view_diplomas.html", context)
