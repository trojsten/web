...
class UserFormTests(TestCase):
    ...
    def test_mailing_option_other(self):
        ...
        self.assertEqual(
            [
                user.mailing_address.street,
                user.mailing_address.town,
                user.mailing_address.postal_code,
                user.mailing_address.country,
            ],
            [
                data["corr_street"],
                data["corr_town"],
                data["corr_postal_code"],
                data["corr_country"],
            ],
        )
...
class SettingsViewTests(TestCase):
    ...
    def test_staff_see_all_props(self):
        staff_user = User.objects.create_user(
            username="staff", first_name="Staff", last_name="Staff", is_staff=True
        )
...
class PeopleApiTests(TestCase):
    ...
    def test_ignore_competition(self):
        ...
        self.client.post(self.url, {"competition": str(self.competition.pk), "value": "true"})
        self.assertTrue(self.user.is_competition_ignored(self.competition))

    def test_unignore_competition(self):
        self.client.force_login(self.user)
        self.client.post(self.url, {"competition": str(self.competition.pk), "value": "false"})
    ...
    def test_ignore_competition_anonymous(self):
        response = self.client.post(
            self.url, {"competition": str(self.competition.pk), "value": "true"}
        )
...
class DeenvelopingTests(TestCase):
    def setUp(self):
        ...
        competition = Competition.objects.create(
            name="TestCompetition", pk=7, organizers_group=group
        )
        ...
        for i in range(1, 11):
            task = Task.objects.create(
                number=i, name="Test task {}".format(i), round=self.round, description_points=9
            )
        ...
        self.user_with_submits = User.objects.create(
            username="submituser",
            password="password",
            first_name="Janko",
            last_name="Hrasko",
        ...
        self.url_new = reverse(
            "admin:submitted_tasks", kwargs={"user_pk": self.new_user.pk, "round_pk": self.round.pk}
        )
        self.url_submits = reverse(
            "admin:submitted_tasks",
            kwargs={"user_pk": self.user_with_submits.pk, "round_pk": self.round.pk},
        )
    ...
    def test_delete_paprer_submit(self):
        self.client.force_login(self.staff_user)
        response = self.client.post(
            self.url_submits, {"7": DEENVELOPING_NOT_REVIEWED_SYMBOL}, follow=True
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            Submit.objects.filter(
    ...
    def test_not_delete_electronic_submit(self):
        self.client.force_login(self.staff_user)
        response = self.client.post(
            self.url_submits, {"1": DEENVELOPING_NOT_REVIEWED_SYMBOL}, follow=True
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            Submit.objects.filter(
    ...
    def test_add_and_delete_submits(self):
        ...
        self.assertEqual(
            Submit.objects.filter(
                user=self.user_with_submits, task__number=1, submit_type=SUBMIT_TYPE_DESCRIPTION
            ).count(),
            0,
        )
...
class AdditionalRegistrationFormTest(TestCase):
    ...
    def test_field_name(self):
        self.assertEqual(
            "user_prop_%s" % self.key1.pk, AdditionalRegistrationForm._field_name(self.key1)
        )
    ...
    def test_clean_bad_regex(self):
        ...
        form = AdditionalRegistrationForm(
            self.user, keys, {AdditionalRegistrationForm._field_name(self.key1): "hello"}
        )
        self.assertFalse(form.is_valid())
    ...
    def test_clean_missing_value(self):
        ...
        form = AdditionalRegistrationForm(
            self.user, keys, {AdditionalRegistrationForm._field_name(self.key2): "hello"}
        )
        self.assertFalse(form.is_valid())
...
class AdditionalRegistrationHelpersTest(TestCase):
    ...
    def test_required_properties_by_competition(self):
        self.assertDictEqual(
            {self.competition: {self.key3}, self.competition2: {self.key4, self.key6}},
            get_required_properties_by_competition(self.user),
        )

    def test_required_properties(self):
        self.assertSetEqual({self.key3, self.key4, self.key6}, get_required_properties(self.user))
...
class UserPropsFormSetTests(TestCase):
    ...
    def test_empty_key_is_not_valid(self):
        f = _get_user_props_formset(True)(
            data={
