...
class TrojstenUserBaseForm(forms.ModelForm):
    ...
    postal_code = forms.CharField(max_length=16, label=_("Postal code"))
    ...
    mailing_option = forms.ChoiceField(
        ...
        label=_("Correspondence address"),
    ...
    corr_postal_code = forms.CharField(max_length=16, label=_("Postal code"), required=False)
    corr_country = LazyTypedChoiceField(choices=countries, label=_("Country"), required=False)
    ...
    class Meta:
        ...
        widgets = {"school": forms.Select(attrs={"class": "autocomplete"})}
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["gender"] = forms.ChoiceField(
            widget=forms.RadioSelect, label=_("Gender"), choices=User.GENDER_CHOICES
        )
    ...
    def clean_email(self):
        if len(self.cleaned_data.get("email")) == 0:
            raise forms.ValidationError(_("This field is required."), code="email_required")
        return self.cleaned_data.get("email")

    def clean_first_name(self):
        if len(self.cleaned_data.get("first_name")) == 0:
            raise forms.ValidationError(_("This field is required."), code="first_name_required")
        return self.cleaned_data.get("first_name")

    def clean_last_name(self):
        if len(self.cleaned_data.get("last_name")) == 0:
            raise forms.ValidationError(_("This field is required."), code="last_name_required")
        ...
        if self.cleaned_data.get("mailing_option") == constants.MAILING_OPTION_OTHER:
            if len(self.cleaned_data.get("corr_street")) == 0:
                raise forms.ValidationError(
                    _("This field is required."), code="corr_street_required"
                )
    ...
    def clean_corr_town(self):
        if self.cleaned_data.get("mailing_option") == constants.MAILING_OPTION_OTHER:
            if len(self.cleaned_data.get("corr_town")) == 0:
                raise forms.ValidationError(_("This field is required."), code="corr_town_required")
    ...
    def clean_corr_postal_code(self):
        if self.cleaned_data.get("mailing_option") == constants.MAILING_OPTION_OTHER:
            if len(self.cleaned_data.get("corr_postal_code")) == 0:
                raise forms.ValidationError(
                    _("This field is required."), code="corr_postal_code_required"
                )
    ...
    def clean_corr_country(self):
        if self.cleaned_data.get("mailing_option") == constants.MAILING_OPTION_OTHER:
            if len(self.cleaned_data.get("corr_country")) == 0:
                raise forms.ValidationError(
                    _("This field is required."), code="corr_country_required"
                )
    ...
    def clean_graduation(self):
        ...
        if grad < constants.GRADUATION_YEAR_MIN:
            raise forms.ValidationError(
                _("Your graduation year is too far in the past."), code="graduation_too_soon"
            )

        if grad > timezone.localtime(timezone.now()).year + constants.GRADUATION_YEAR_MAX_AHEAD:
            raise forms.ValidationError(
                _("Your graduation year is too far in the future."), code="graduation_too_late"
            )
    ...
    def clean_mailing_option(self):
        ...
        if option == constants.MAILING_OPTION_SCHOOL and school is None:
            raise forms.ValidationError(
                _(
                    "We cannot send you correspondence to school when you don't choose any school. "
...
class TrojstenUserCreationForm(TrojstenUserBaseForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.password_required:
            ...
            self.fields["password1"].help_text = format_lazy(
                "{}{}",
                _(
...
class MergeFieldFactory:
    ...
    def get_field(self, key, display=False, use_datetime=False):
        def get_choice(user):
            return (
                user.pk,
                getattr(user, "get_%s_display" % (key,))() if display else getattr(user, key),
            )
    ...
    def get_prop_field(self, key):
        def get_choice(user):
            return (user.pk, self.props[user].get(key, None))
                ...
                [
                    (
                        ...
                        field_factory.get_field(
                            f, display=f in display, use_datetime=f in use_datetime
                        ),
                    )
            ...
            if len(value) > 0:
                try:
                    ...
                    if points < 0 or points > self.max_points[int(task_number)]:
                        self.add_error(
                            task_number,
                            _("Points have to be non-negative and at most {}.").format(
                                self.max_points[int(task_number)]
                            ),
                        )
                except ValueError:
                    if value != constants.DEENVELOPING_NOT_REVIEWED_SYMBOL:
                        self.add_error(
                            task_number,
                            _("The value has to be number or {}.").format(
                                constants.DEENVELOPING_NOT_REVIEWED_SYMBOL
                            ),
                        )
        return cleaned_data
    ...
    def save(self, user):
        for task in Task.objects.filter(round=self.round):
            ...
            if len(value) > 0:
                ...
                submit = (
                    Submit.objects.filter(task=task, user=user, submit_type=SUBMIT_TYPE_DESCRIPTION)
                    .order_by("-time")
                    .first()
                )
                if submit:
...
class IgnoreCompetitionForm(forms.Form):
    ...
    def save(self):
        ...
        participating_competitions = set(
            filter(lambda c: c.pk in participating_competitions_pk, competitions)
        )
                ...
                (
                    ...
                    forms.CharField(
                        ...
                        widget=forms.widgets.Textarea(
                            attrs={"class": "col-sm-12 form-control", "rows": 1}
                        ),
    ...
    def clean(self):
        ...
        for prop_key in self.prop_keys:
            ...
            cleaned_data[field_name] = UserProperty(
                user=self.user, key=prop_key, value=cleaned_data.get(field_name)
            )
            try:
