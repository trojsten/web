...
class ContactFormTests(TestCase):
    ...
    def test_valid_data_required(self):
        ...
        """
        data = {"name": "Test", "body": "Test message", "email": "test@example.com"}
    ...
    def test_send(self):
        ...
        """
        form = ContactForm(request=self.request(), captcha=False, data=self.valid_data)
        ...
        self.assertEqual(settings.CONTACT_FORM_RECIPIENTS, tuple(message.recipients()))
        self.assertListEqual(
            ["{}<{}>".format(self.valid_data["name"], self.valid_data["email"])], message.reply_to
        )
