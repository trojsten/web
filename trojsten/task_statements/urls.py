...
urlpatterns = [
    ...
    url(r'^solutions_pdf/(?P<round_id>\d+)/', views.view_pdf,
        {'solution': True}, name='view_solutions_pdf'),
    url(r'^(?P<type>(zadania)|(riesenia))/(?P<task_id>\d+)/%s/(?P<picture>.+)$'
        % settings.TASK_STATEMENTS_PICTURES_DIR, views.show_picture, name='show_picture'),
    url(r'^ajax/(?P<round_id>\d+)/progressbar.html$', views.ajax_progressbar, name='ajax_progressbar'),
]
