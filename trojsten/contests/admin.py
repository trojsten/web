...
class RoundAdmin(admin.ModelAdmin):
    ...
    get_semester_number = get_related(
        attribute_chain=("semester", "number"), description="časť", order="semester__number"
    )
    get_semester_name = get_related(
        attribute_chain=("semester", "name"), description="názov časti", order="semester__name"
    )
    get_year = get_related(
        attribute_chain=("semester", "year"), description="ročník", order="semester__year"
    )
    ...
    tasks_pdf = attribute_format(
        attribute="tasks_pdf_exists", description="zadania v pdf", boolean=True
    )
    solutions_pdf = attribute_format(
        attribute="solutions_pdf_exists", description="vzoráky v pdf", boolean=True
    )
    ...
    def get_queryset(self, request):
        ...
        return super(RoundAdmin, self).get_queryset(request).filter(semester__in=semester_lst)
...
class TaskByYearSubFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        tasks = tasks.distinct("round__semester__year").order_by("-round__semester__year")
...
class TaskByRoundSubFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        if "year_subfilter" in request.GET:
            tasks = tasks.filter(round__semester__year=request.GET["year_subfilter"])
...
class CategoryAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        ...
        return (
            ...
            .get_queryset(request)
            .filter(competition__organizers_group__in=user_groups)
