        ...
        tests = [
            {
                ...
                "showDetails": runtest.details is not None
                and ("sample" in runtest.name or force_show_details),
            }
...
def view_reviewer_comment(request, submit_id):
    ...
    if (
        ...
        and not Submit.objects.filter(
            pk=submit.pk,
            task__round__semester__competition__organizers_group__user__pk=request.user.pk,
        ).exists()
    ):
...
def view_protocol(request, submit_id):
    ...
    if (
        ...
        return render(request, "trojsten/submit/protocol.html", template_data)
    else:
        raise Http404
...
def view_submit(request, submit_id):
    ...
    # For description submits, return submitted file.
        ...
        if os.path.exists(submit.filepath):
            return sendfile(request, submit.filepath, attachment=send_attachment)
        else:
            raise Http404  # File does not exists, can't be returned

    else:
        return render(
            request, "trojsten/submit/view_submit.html", {"submit": submit, "source": False}
        )
...
def active_rounds_submit_page(request):
    ...
    return render(request, "trojsten/submit/active_rounds_submit.html", template_data)
...
def all_submits_page(request, submit_type):
    ...
    return render(request, "trojsten/submit/all_submits_page.html", context)
    ...
    send_mail(
        ...
        (
            ...
            + (
                ...
                if submit_type == constants.SUBMIT_TYPE_DESCRIPTION
                else ""
            )
            ...
            task=submit.task,
            submit_link=Site.objects.get_current().domain
            + reverse("admin:old_submit_submit_change", args=(submit.id,)),
            review_link=Site.objects.get_current().domain
            + reverse("admin:review_edit", args=(task_id, submit.id)),
        ),
        settings.DEFAULT_FROM_EMAIL,
        [
            org.email
            for org in submit.task.get_assigned_people_for_role(contest_consts.TASK_ROLE_REVIEWER)
        ],
    )
